...
#
# Customized agnoster theme fo oh-my-zsh
...
prompt_context() {
  if [[ "$USERNAME" != "$DEFAULT_USER" || -n "$SSH_CLIENT" ]]; then
    prompt_segment "$AGNOSTER_CONTEXT_BG" "$AGNOSTER_CONTEXT_FG" " %(!.%{%F{$AGNOSTER_STATUS_ROOT_FG}%}.)%B%n@%m%b"
  fi
}
...
prompt_status() {
  ...
  [[ $UID -eq 0 ]] && symbols+="%{%F{$AGNOSTER_STATUS_ROOT_FG}%}⚠️ "
...
get_virtualenv() {
  if [ -n "$VIRTUAL_ENV" ]; then
    echo "%F{blue} venv: (${VIRTUAL_ENV:t:gs/%/%%})%f"
...
RPROMPT='$(get_virtualenv) %{$fg[green]%}[ %*]%{$reset_color%}'
